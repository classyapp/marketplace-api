...
namespace classy.Manager
{
    public class DefaultProfileManager : IProfileManager, IReviewManager
    {
            ...
            string culture)
        {
            ...
            {
                results.Add(profile.ToProfileView().ToAPIModel().Include(x => x.ProfessionalInfo, x => x.ContactInfo, x => x.Id, x => x.IsProfessional, x => x.IsProxy, x => x.IsVendor, x => x.IsVerifiedProfessional, x => x.ListingCount, x => x.Listings, x => x.Metadata));
            }
