...
{
    /*
    public class DictionaryAuthTestRepository : IUserAuthRepository, IClearable
    {
        ...
        public Regex ValidUserNameRegEx = new Regex(@"^(?=.{3,45}$)([A-Za-z0-9][._-]?)*$", RegexOptions.Compiled);
         ...
         Dictionary<string, UserAuth> usersDB = new Dictionary<string, UserAuth>();
        ...
        private static UserAuth GetUserAuthByUserName(string appId, string userNameOrEmail)
        {
        ...
        private static void AssertNoExistingUser(UserAuth newUser, UserAuth exceptForExistingUser = null)
        {
            if (newUser.UserName != null)
            {
                var existingUser = GetUserAuthByUserName(newUser.AppId, newUser.UserName);
            ...
            {
                var existingUser = GetUserAuthByUserName(newUser.AppId, newUser.Email);
                if (existingUser != null
                    && (exceptForExistingUser == null || existingUser.Id != exceptForExistingUser.Id))
    ...
    /*
    public class MongoDBAuthTestRepository : IUserAuthRepository, IClearable
    {
        ...
        public MongoDBAuthTestRepository(MongoDatabase db, bool createMissingCollections)
        {
