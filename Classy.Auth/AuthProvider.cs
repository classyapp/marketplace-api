...
namespace Classy.Auth
{
    public abstract class AuthProvider : IAuthProvider
    {
        ...
        public virtual void OnAuthenticated(IServiceBase authService, IAuthSession session, IOAuthTokens tokens, Dictionary<string, string> authInfo)
        {
            if (session != null)
            {
                LoadUserAuthInfo(session, tokens, authInfo);
            }
        ...
        protected virtual void LoadUserAuthInfo(IAuthSession userSession, IOAuthTokens tokens, Dictionary<string, string> authInfo) { }
